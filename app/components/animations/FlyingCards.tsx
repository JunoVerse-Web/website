...
export default function FlyingCards({
	...
	const flyAway = useCallback(() => {
		...
		card.style.transition = "transform 1.1s cubic-bezier(0.25, 0.1, 0.25, 1), opacity 0.9s ease-out";
	...
	const resetCard = useCallback(() => {
		...
		const card = cardRef.current;
		card.style.transition = "transform 0.9s cubic-bezier(0.34, 1.56, 0.64, 1), opacity 0.8s ease-out";
	...
	return (
		...
		>
			...
			<div className="absolute inset-x-0 top-[25%] max-w-[90%] lg:max-w-[70%] mx-auto">
				{text?.map((p, i) => (
					<p key={i} className="text-white text-center mb-0!">
						{p}
					</p>
				))}
			</div>
